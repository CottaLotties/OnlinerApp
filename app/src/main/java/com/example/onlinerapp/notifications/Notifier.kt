...
class Notifier(context: Context, workerParams: WorkerParameters) : Worker(context, workerParams) {
    ...
    private fun sendNotification() {
        ...
        val resultIntent = Intent(applicationContext, MainActivity::class.java).putExtra("FRAGMENT", "CART")
        val resultPendingIntent = PendingIntent.getActivity(applicationContext, 0, resultIntent,
        ...
        val actionPendingIntent = PendingIntent
                .getBroadcast(applicationContext, 1, removeActionIntent, PendingIntent.FLAG_UPDATE_CURRENT)

        val notification = NotificationCompat.Builder(mContext, channelId)
            .addAction(R.drawable.outline_delete_white_24, "Очистить корзину", actionPendingIntent)
